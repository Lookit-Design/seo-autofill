<?php
/**
 * Keyphrase Engine – Lookit SEO Autofill
 *
...
class ASY_Keyphrase_Engine {
    ...
    public static function get_related_keyphrases( WP_Post $post, $count = 3 ) {
        ...
        // Remove phrases that are identical to the focus keyword
        $focus = strtolower( sanitize_text_field( $post->post_title ) );
        $all   = array_filter( $all, function( $kp ) use ( $focus ) {
            return strtolower( $kp ) !== $focus;
        } );

        $all = array_values( array_slice( $all, 0, $count ) );
    ...
    private static function extract_from_content( WP_Post $post, $limit = 9 ) {
        ...
        $words      = array_values( array_filter( $words, function( $w ) use ( $stop_words ) {
        ...
        foreach ( $ngrams as $phrase => $freq ) {
            ...
            $score = $freq;
            if ( $in_title )   $score *= 2.0;
            if ( $in_heading ) $score *= 1.5;
    ...
    private static function walk_elementor_elements( array $elements, array &$collected ) {
        ...
        static $text_keys = array(
            'title', 'text', 'editor', 'description', 'content', 'caption',
            'html', 'button_text', 'heading', 'sub_title', 'subtitle', 'label',
            'body', 'excerpt', 'inner_text', 'heading_text', 'text_content',
            'item_description',
        );
    ...
    private static function datamuse_expand( $seed, $content_phrases, $limit = 6 ) {
        ...
        $ml_results = self::datamuse_query( array( 'ml' => $seed, 'max' => 20 ) );
        // Query 2: "triggered by" — words/phrases strongly associated with seed
        $rel_results = self::datamuse_query( array( 'rel_trg' => $seed_encoded, 'max' => 20 ) );
        ...
        foreach ( $candidates as $item ) {
            ...
            if ( strpos( $word, ' ' ) !== false ) {
                $multi[] = array( 'word' => $word, 'score' => $score * 1.5 );
            } else {
                // Pair with a content phrase word to make a bigram
                $single[] = array( 'word' => $word, 'score' => $score );
            }
        }
        ...
        foreach ( $single as $item ) {
            ...
            if ( isset( $stop[ $w ] ) || mb_strlen( $w ) < 3 ) continue;
            $multi[] = array(
                'word'  => $seed_word . ' ' . $w,
                'score' => $item['score'],
            );
        ...
        foreach ( $multi as $item ) {
            ...
            if ( in_array( strtolower( $phrase ), $existing, true ) ) continue;
            $results[] = $phrase;
    ...
    private static function datamuse_query( $params ) {
        ...
        $response = wp_remote_get( $url, array(
            'timeout'    => 8,
            'user-agent' => 'Lookit SEO Autofill/' . ASY_VERSION . ' (+' . get_site_url() . ')',
        ) );
    ...
    private static function stop_words() {
        ...
        $words = array(
            ...
            'yourselves','also','just','like','new','one','use','used','using',
            'well','will','may','might','shall','even','still','now','then','here',
            'there','back','way','take','make','good','know','think','see','come',
            'want','give','look','first','last','long','great','little','own',
            'right','high','place','large','next','early','young','important',
            'public','private','real','best','free','used','need','help','find',
            'its','our','their','has','are','was','were','been','being','have',
            'had','does','did','able','across','already','always','another',
            'around','became','become','becomes','becoming','behind','beside',
            'besides','beyond','call','came','cannot','certain','changes','clear',
            'come','consider','considered','either','else','enough','especially',
            'fact','far','felt','form','four','full','general','given','goes',
            'got','hand','heard','hence','however','including','indeed','kept',
            'kind','known','later','least','less','let','likely','made','main',
            'many','maybe','means','might','moved','much','must','near','never',
            'non','nothing','often','old','once','open','order','part','perhaps',
            'point','possible','probably','put','quite','rather','really','said',
            'second','seen','set','several','she','side','since','small','something',
            'sometimes','soon','stand','still','sure','take','taken','though',
            'three','thus','together','told','towards','tried','true','try','turn',
            'two','upon','usually','various','without','yet',
        );
