...
class ASY_Processor {
    ...
    public function process( $post ) {
        ...
        if ( in_array( $post->ID, self::$processed_ids, true ) ) return;
        ...
        if ( empty( $templates[ $pt ]['enabled'] ) ) return;
    ...
    public function save_lock_from_rest( $post, $request ) {
        ...
        if ( ! isset( $params['meta']['_asy_seo_locked'] ) ) return;
    ...
    private function truncate( $string, $max ) {
        if ( mb_strlen( $string ) <= $max ) return $string;
